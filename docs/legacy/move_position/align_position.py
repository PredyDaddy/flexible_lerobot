...
NOTE (lerobot 0.4.3 baseline):
This script depends on `lerobot.cameras.ros_camera`, which is not included in the
current repository's baseline code. It is kept as a legacy reference; to make it
work on 0.4.3 you would need to port the ROS camera module or refactor this
script to use an available camera backend (e.g. OpenCV/RealSense/ZMQ).

Usage:
    python docs/legacy/move_position/align_position.py \
