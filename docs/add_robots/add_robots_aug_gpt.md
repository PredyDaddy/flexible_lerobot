...
参考：`src/lerobot/robots/so_follower/config_so_follower.py`、`lekiwi/config_lekiwi.py`。
...
- `src/lerobot/teleoperators/so_leader/`（Teleoperator type: `so100_leader` / `so101_leader`）；
...
- `tests/robots/test_so100_follower.py`
  ...
  - 使用 `patch("lerobot.robots.so_follower.so100_follower.FeetechMotorsBus", ...)` 注入测试总线；
