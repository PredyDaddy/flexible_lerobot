...
## 我的开发说明
这个仓库是我基于lerobot仓库开发出来的一个机器人框架, 我主要的开发是放在my_devs 这个路径下面，然后my_devs/docs 每一个功能点我都会单独做成一个文件夹，然后也会单独在d OCS里面写这个的开发文档跟报告文档

## 开发规范

- 必须使用 `lerobot_flex` 这个 conda 环境开发代码；任何测试、格式化、lint、脚本执行也必须使用这个环境。
- 代码开发默认放在 `my_devs/`；尽量不要在其他模块直接开发，除非获得用户明确授权。
- 使用子代理的时候必须使用gpt-5.4 xhigh