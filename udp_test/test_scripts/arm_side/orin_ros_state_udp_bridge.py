        ...
        *,
        ...
        with self._lock:
            ...
            if len(receive_monotonic_values) == len(SOURCE_NAMES):
                source_skew_ms = (
                    max(receive_monotonic_values) - min(receive_monotonic_values)
                ) / 1_000_000
            ...
            if require_all_sources_advanced and any(
                generations[name] <= last_sent_generations.get(name, 0) for name in SOURCE_NAMES
            ):
...
class StateUdpSender:
    def __init__(
        ...
        *,
        ...
        self.last_sent_generations = {name: 0 for name in SOURCE_NAMES}
    ...
    def _metrics_text(self, snapshot: StateSnapshot) -> str:
        ...
        return (
            f"requested_hz={self.formatted_hz} actual_hz={self.formatted_hz} "
    ...
    def attempt_send(self) -> bool:
        ...
        if self.counters.sent == 1 or (
            self.print_every > 0 and self.counters.sent % self.print_every == 0
        ):
            self.printer(
                "[orin ros state udp bridge] "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hz", type=_positive_float, default=30.0)
...
def main() -> int:
    ...
    try:
        ...
        print(
            ...
            f"target={target[0]}:{target[1]} requested_hz={args.hz:g} actual_hz={args.hz:g} "
