...
read_bridge_hz() {
  ...
  for ((index = 0; index < ${#argv[@]}; index++)); do
    if [[ "${argv[index]}" == "$BRIDGE_SCRIPT" || "${argv[index]}" == */"$BRIDGE_SCRIPT" ]]; then
      found_bridge=true
      break
    fi
  done
...
find_bridge_process() {
  ...
  while IFS= read -r candidate; do
    ...
    if actual_hz="$(read_bridge_hz "$candidate")"; then
      printf '%s %s\n' "$candidate" "$actual_hz"
      return 0
    fi
...
while ((SECONDS < deadline)); do
  ...
  if read -r BRIDGE_PID ACTUAL_HZ_RAW < <(find_bridge_process "$LAUNCHER_PID"); then
    break
  fi
  sleep 0.1
done

if [[ -z "$BRIDGE_PID" || -z "$ACTUAL_HZ_RAW" ]]; then
  echo "[orin_arm/start] timed out locating the Python bridge process" >&2
  stop_failed_launch "$LAUNCHER_PID"
  exit 1
fi
ACTUAL_HZ="$(normalize_hz "$ACTUAL_HZ_RAW")"
if ! hz_equal "$ACTUAL_HZ" "$EXPECTED_HZ_NORMALIZED"; then
  echo "[orin_arm/start] bridge argv mismatch requested_hz=$REQUESTED_HZ_NORMALIZED actual_hz=$ACTUAL_HZ expected_hz=$EXPECTED_HZ_NORMALIZED" >&2
  stop_failed_launch "$LAUNCHER_PID"
  exit 1
fi
...
HZ_PATTERN="requested_hz=$REQUESTED_HZ_NORMALIZED actual_hz=$ACTUAL_HZ"
while ((SECONDS < deadline)); do
...
if ! grep -Fq "$STARTUP_PATTERN" "$LOG_FILE" || ! grep -Fq "$HZ_PATTERN" "$LOG_FILE"; then
  echo "[orin_arm/start] bridge log hz verification failed requested_hz=$REQUESTED_HZ_NORMALIZED actual_hz=$ACTUAL_HZ" >&2
  tail -n 20 "$LOG_FILE" >&2 || true
  stop_failed_launch "$LAUNCHER_PID"
  exit 1
fi
...
echo "[orin_arm/start] verified bridge pid=$BRIDGE_PID launcher_pid=$LAUNCHER_PID requested_hz=$REQUESTED_HZ_NORMALIZED actual_hz=$ACTUAL_HZ"
