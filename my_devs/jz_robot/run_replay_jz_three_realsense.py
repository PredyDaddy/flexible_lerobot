...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="One-click JZRobot dual-arm + gripper replay entrypoint."
    )
    ...
    parser.add_argument("--use-external-commands", type=parse_bool, nargs="?", const=True, default=False)
    ...
    parser.add_argument("--connect-cameras", type=parse_bool, nargs="?", const=True, default=False)

    parser.add_argument(
        "--dataset-repo-id",
        default=env_default("DATASET_REPO_ID", "local/jz_dual_arm_three_rs"),
    )
    parser.add_argument(
        "--dataset-root",
        default=env_default("DATASET_ROOT", "/home/test/data/lerobot"),
    )
    parser.add_argument("--episode", type=int, default=int(env_default("EPISODE", "0")))
...
def main() -> None:
    ...
    print("[INFO] JZRobot quick replay config")
