# PI05 本地 Chunk 实时运行时技术方案

本稿已根据 `reviews/review_01.md`、`reviews/review_02.md`、`reviews/review_03.md` 的独立评审意见完成收敛修订。

## 0. 文档口径

这份文档讨论的是 **第一阶段的 PI05 本地 Chunk 实时运行时**，不是仓库中通用的 `src/lerobot/async_inference/` gRPC server/client 方案。

这里提到的“异步”，默认指的是：

- 同一进程内
- `producer` / `actor` 双线程
- 用 `ActionQueue` 解耦推理与执行
...
> 如何把当前同步单步的 `run_pi05_infer.py`，演进成一个本地双线程的 PI05 chunk runtime，并为后续 RTC 与第二阶段 async_inference 对接打基础。

另外，这份文档有两个明确工程边界：

1. 第一阶段新增代码目录固定在 `my_devs/pi05_engineering/`
2. 未经用户确认，不执行任何上机动作；实机验证只作为最后由用户执行的验收阶段

## 1. 结论先行

本轮方案的最终结论不是“直接把当前 `run_pi05_infer.py` 改成通用 `async_inference`”，也不是“继续在同步单循环上打补丁”，而是采用一条分阶段、但目标明确的路线：

1. 第一阶段先在 `my_devs/pi05_engineering/` 内落一个 **PI05 本地 chunk runner**。
...
> 先把当前已经能跑起来的 PI05 实机推理，升级成“本地双线程 chunk + 可观测 + 可切换 RTC”的稳定运行时；再决定是否把这套运行时语义映射到通用 `async_inference` 的 server/client 模式。
...
### 2.2 当前脚本是同步单循环，且调用方走的是单动作 API 路径
...
- 调用方每轮只拿一个 action
- policy 内部仍可能按 chunk 生成并缓存动作
- 调用方对 chunk 调度、leftover、delay、refill 时机没有显式控制权
...
- 从 checkpoint 目录加载 `policy_preprocessor.json` / `policy_postprocessor.json`
- 在 repo root 下显式检查本地 tokenizer 目录兜底

因此代码层面可以确认，baseline 当前显式选择的“已知可用行为”是：

- 非严格加载
- 使用 checkpoint 内保存的 pre/post processor
- 使用脚本层面的本地 tokenizer 目录兜底

这里要把强度说准确：

- 仅凭静态代码审查，不能直接证明“当前 checkpoint 必须依赖 strict=False 才能跑通”
- 当前可以确定的是：baseline 显式使用了 `strict=False`，而通用 `async_inference` 默认并不保留这一点
- 因此任何新的运行时方案都应先与 baseline 的**显式加载行为**对齐，而不是擅自改写
...
- 在 `my_devs/pi05_engineering/` 里实现一个 **PI05 本地实时 chunk runner**
...
- `async_inference` 当前其实已经会从同一 checkpoint 路径加载 pre/post processor
- 因此它与 baseline 的差异，不在“processor 文件来源”
- 真正还没有对齐的是：
  - `strict=False`
  - baseline 的 repo-root / 本地 tokenizer fail-fast 行为
  - 当前 so101 wiring 与 runtime 数据流

所以如果直接切换到 generic async stack，风险主要来自**加载细节和运行时语义偏移**，而不只是 processor 文件本身
...
这两个合在一起，足够构成第一阶段方案的主骨架。

但需要明确边界：

- `run_pi05_infer.py` 提供的是当前 baseline 的加载与 robot wiring 事实
- `examples/rtc/eval_with_real_robot.py` 提供的是 producer / actor 拆分、RTC 调用方式、ActionQueue / LatencyTracker 的参考
- 它并没有完整提供 first-chunk barrier、`hold-last-action`、统一异常停机等第一阶段安全 contract
...
- 现有 `ActionQueue` 在 RTC enabled 时，会按 `real_delay` 做 replace
- 现有 `ActionQueue` 在 RTC disabled 时，只会 append，并不会自动修剪“新 chunk 中已过期的 prefix”
- 因此如果第一阶段 plain chunk 也要求 stale-prefix trimming、空队列安全策略、更细粒度日志，那么这些能力应由 runtime 外层 wrapper 明确补齐
...
- `examples/rtc` 当前真正传给 `ActionQueue.merge(...)` 的，是 `new_delay = ceil(new_latency / dt)`
- `action_index_before_inference` 在现有 `ActionQueue` 里主要用于一致性校验和 warning

因此更准确的表述应是：

- 第一版如果沿用仓库现有实现，queue merge 的直接输入仍是 wall-clock 推导出的 `new_delay`
- action index delta 应作为交叉校验、监控和问题定位指标
- 如果后续要改成“index delta 是 authoritative delay”，那应明确标注为新增实现，而不是当前仓库已具备的事实
...
要让 RTC 真正生效，最好把“policy 直接输入”和“runtime bookkeeping”分开描述。

对 `predict_action_chunk()` 直接相关的 RTC 输入主要是：
...
这些状态同样重要，但它们更多属于 runtime / queue bookkeeping，而不是 `predict_action_chunk()` 的直接必填参数
...
- 下面这些条目是第一阶段必须新增落实的 runtime contract
- 不能把它们理解成“仓库当前参考代码已经完整提供”
...
## 6.1 新增代码应主要放在 `my_devs/pi05_engineering/`

按用户最新要求，第一阶段新增代码统一放在：

- `my_devs/pi05_engineering/`

因此建议新增一个 PI05 实时运行模块，形态可以是：

- `my_devs/pi05_engineering/runtime/`
- `my_devs/pi05_engineering/run_pi05_chunk_infer.py`

其中：

- `runtime/` 负责 runtime/library
- `run_pi05_chunk_infer.py` 负责 CLI 入口
...
2. 在 `my_devs/pi05_engineering/` 下新增实时版本入口
...
这些事情在 `my_devs/pi05_engineering/` 内就能完成。
...
- `examples/rtc/eval_with_real_robot.py` 更适合复用“线程职责拆分、RTC 调用模式、延迟与 queue 指标口径”
- 它并不等于“完整可直接搬用的安全运行时合同”
...
由于当前脚本显式使用 `strict=False`，而通用加载路径默认并不保留这一点：
...
- 上述大部分指标不是当前 baseline 已经具备的现成日志
- 它们是新 runtime 需要新增的 instrumentation
- 因此这些指标更适合作为“新 runtime 的验收门槛”，而不是与当前 baseline 的直接同口径对照
...
- 因此“plain chunk 比 baseline starvation 更少”这类判断，只有在 baseline 额外补同口径 instrumentation 后才具备严格可比性
...
- 关闭 RTC
- 只验证 chunk queue、delay、startup、safety 逻辑是否稳定
- 重点看 warmup、starvation、steady-state FPS

### 档位 B：用户确认后的上机 plain chunk / RTC enabled

- 先由用户上机确认 plain chunk
- 打开 RTC
...
2. 在 `my_devs/pi05_engineering/` 下新增 PI05 实时运行时入口与 runtime 模块。
3. 第一阶段直接复用：
   - `ActionQueue`
   - `LatencyTracker`
...
- 复用的是底层原语和线程职责，不是默认继承其全部 plain-mode queue 语义与安全 contract
