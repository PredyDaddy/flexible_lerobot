...
class RemotePi05Client:
    ...
    def health(self) -> dict[str, Any]:
        ...
        response = self.session.get(health_url, timeout=self.timeout_s)
        response.raise_for_status()
        return response.json()

    def reset(self) -> dict[str, Any]:
        reset_url = f"{self.base_url}/reset"
        response = self.session.post(reset_url, timeout=self.timeout_s)
        response.raise_for_status()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="PI0.5 async robot client. Sends observations to a remote server and executes actions."
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        const=True,
        default=env_bool("SKIP_HEALTH_CHECK", False),
    )
    ...
    parser.add_argument(
        "--top-cam",
        type=parse_camera,
        default=parse_camera(os.getenv("TOP_CAM", "/dev/video4")),
    )
    parser.add_argument(
        "--wrist-cam",
        type=parse_camera,
    ...
    parser.add_argument(
        ...
        help="Exact language instruction sent to server. Required unless --task-id is used.",
    )
    parser.add_argument(
        "--task-id",
    ...
    parser.add_argument(
        ...
        type=int,
        default=int(os.getenv("INFERENCE_OVERLAP_STEPS", "0")),
    )
    parser.add_argument(
        ...
        help="Prefetch the next remote action chunk in a background thread at the overlap point.",
    )
    parser.add_argument("--chunk-blend-steps", type=int, default=int(os.getenv("CHUNK_BLEND_STEPS", "0")))
    parser.add_argument(
        ...
        help="Read camera/state only when a new remote chunk request is needed.",
    )
    parser.add_argument(
        "--future-state-aware",
        type=parse_bool,
        nargs="?",
        const=True,
        default=env_bool("FUTURE_STATE_AWARE", False),
    )
    parser.add_argument("--action-quant-ratio", type=int, default=int(os.getenv("ACTION_QUANT_RATIO", "1")))
    parser.add_argument(
        "--max-action-abs",
        type=optional_float,
        default=optional_float(os.getenv("MAX_ACTION_ABS")),
    )
    parser.add_argument(
        "--max-action-delta",
        type=optional_float,
        default=optional_float(os.getenv("MAX_ACTION_DELTA")),
    )
    return parser
...
def resolve_task(parser: argparse.ArgumentParser, args: argparse.Namespace) -> str:
    ...
    if task is None:
        parser.error(
            "A task must be specified for multi-task PI0.5 remote inference. "
...
def main() -> None:
    ...
    if args.action_quant_ratio != 1:
        raise NotImplementedError(
            "action_quant_ratio > 1 is intentionally disabled in this remote async implementation. "
    ...
    print_runtime_summary(
        repo_root=REPO_ROOT,
        robot_cfg=robot_runtime_cfg,
        policy_path=Path("<remote-server>"),
        task=task,
    )
    print(f"[INFO] Server URL: {args.server_url.rstrip('/')}{args.endpoint}")
    ...
    remote_client = RemotePi05Client(
    ...
    dataset_features, robot_action_processor, robot_observation_processor = (
        make_dataset_features_and_processors(robot)
    )
    ...
    if args.n_action_steps > 0:
        ...
        n_action_steps = int(health.get("n_action_steps") or health.get("chunk_size") or 50)
    print(f"[INFO] client n_action_steps: {n_action_steps}", flush=True)
    ...
    try:
        ...
        for attempt in range(1, connect_attempts + 1):
            try:
                print(
                    f"[INFO] Connecting robot and cameras... attempt {attempt}/{connect_attempts}",
                    flush=True,
                )
                robot.connect()
                break
        ...
        while True:
            if end_t is not None and time.perf_counter() >= end_t:
                print("[INFO] Reached requested run_time_s. Exiting remote inference loop.")
                break
            ...
            if args.log_interval > 0 and step % args.log_interval == 0:
                ...
                print(
                    ...
                    f"request_latency={manager.stats.last_inference_s:.3f}s "
                    f"server_infer={last_server_infer_s:.3f}s "
                    f"request_count={manager.stats.inference_count} "
