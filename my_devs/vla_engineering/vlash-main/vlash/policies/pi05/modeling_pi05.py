...
class PI05Model(nn.Module):
    ...
    def __init__(self, config: PI05Config):
        ...
        if getattr(config.action_expert_config, "use_adarms", False) and (
            getattr(config.action_expert_config, "adarms_cond_dim", None) is None
        ):
        ...
        ensure_gemma_rms_norm_compat(
            self.action_expert.model,
            cond_dim=(
                config.action_expert_config.adarms_cond_dim
                if getattr(config.action_expert_config, "use_adarms", False)
                else None
            ),
        )
