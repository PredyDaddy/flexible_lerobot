...
for value_name in MAX_SOURCE_AGE_MS MAX_SOURCE_SKEW_MS; do
  ...
  if [[ ! "$value" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
    echo "[timed/edge/replay] invalid ${value_name}=${value}; expected a nonnegative number" >&2
    exit 2
  fi
done
...
if [[ ! "$MIN_MEASURED_STATE_HZ_RATIO" =~ ^(0([.][0-9]+)?|1([.]0+)?)$ ]] || \
  ...
  echo "[timed/edge/replay] invalid MIN_MEASURED_STATE_HZ_RATIO=$MIN_MEASURED_STATE_HZ_RATIO" >&2
  exit 2
fi
...
if ! awk -v value="$MAX_SOURCE_AGE_MS" 'BEGIN { exit !(value == 50) }'; then
  if [[ "$JZ_TIMED_SOURCE_AGE_MS_CONFIRM" != "$MAX_SOURCE_AGE_MS" ]]; then
    echo "[timed/edge/replay] refusing MAX_SOURCE_AGE_MS=$MAX_SOURCE_AGE_MS without exact JZ_TIMED_SOURCE_AGE_MS_CONFIRM" >&2
    exit 2
  fi
...
if ! awk -v value="$MAX_SOURCE_SKEW_MS" 'BEGIN { exit !(value == 20) }'; then
  if [[ "$JZ_TIMED_SOURCE_SKEW_MS_CONFIRM" != "$MAX_SOURCE_SKEW_MS" ]]; then
    echo "[timed/edge/replay] refusing MAX_SOURCE_SKEW_MS=$MAX_SOURCE_SKEW_MS without exact JZ_TIMED_SOURCE_SKEW_MS_CONFIRM" >&2
    exit 2
  fi
...
case "${REQUIRE_ALL_SOURCES_ADVANCED,,}" in
  ...
  0|false|no|off)
    if [[ "$JZ_TIMED_REQUIRE_ALL_SOURCES_ADVANCED_CONFIRM" != "$REQUIRE_ALL_SOURCES_ADVANCED" ]]; then
      echo "[timed/edge/replay] refusing REQUIRE_ALL_SOURCES_ADVANCED=$REQUIRE_ALL_SOURCES_ADVANCED without exact JZ_TIMED_REQUIRE_ALL_SOURCES_ADVANCED_CONFIRM" >&2
      exit 2
    fi
    ...
    ;;
  *)
    echo "[timed/edge/replay] invalid REQUIRE_ALL_SOURCES_ADVANCED=$REQUIRE_ALL_SOURCES_ADVANCED" >&2
    exit 2
    ;;
esac
...
if ! awk -v value="$MIN_MEASURED_STATE_HZ_RATIO" 'BEGIN { exit !(value == 0.9) }'; then
  if [[ "$JZ_TIMED_MIN_MEASURED_STATE_HZ_RATIO_CONFIRM" != "$MIN_MEASURED_STATE_HZ_RATIO" ]]; then
    echo "[timed/edge/replay] refusing MIN_MEASURED_STATE_HZ_RATIO=$MIN_MEASURED_STATE_HZ_RATIO without exact JZ_TIMED_MIN_MEASURED_STATE_HZ_RATIO_CONFIRM" >&2
    exit 2
  fi
