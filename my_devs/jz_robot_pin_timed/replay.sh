...
case "${EXECUTION}" in
  dry_run|armed) ;;
  *) echo "[timed/replay] unsupported EXECUTION=${EXECUTION}; use dry_run or armed" >&2; exit 2 ;;
esac

timed_require_armed_confirmation "${EXECUTION}" "replay"
timed_make_python_cmd "${CONDA_ENV}"
export PYTHONDONTWRITEBYTECODE="${PYTHONDONTWRITEBYTECODE:-1}"
export PYTHONPATH="${REPO_ROOT}/src:${PYTHONPATH:-}"

echo "[timed/replay] dataset=${DATASET_ROOT} repo_id=${DATASET_REPO_ID} episode=${EPISODE}"
echo "[timed/replay] robot command: udp://${ORIN_IP}:${COMMAND_PORT} execution=${EXECUTION}"
echo "[timed/replay] cameras disabled during replay; conda_env=${CONDA_ENV} fps=${REPLAY_FPS}"

cd "${REPO_ROOT}"
exec "${PYTHON_CMD[@]}" -m lerobot.scripts.lerobot_replay \
  --robot.type=jz_robot_pin_timed \
  --robot.id=jz_robot_pin_timed_replay \
