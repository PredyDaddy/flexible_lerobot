...
case "${EXECUTION}" in
  dry_run|armed) ;;
  *)
    echo "[timed/x86/control] unsupported EXECUTION=${EXECUTION}; use dry_run or armed" >&2
    exit 2
    ;;
esac

timed_require_armed_confirmation "${EXECUTION}" "control"
timed_make_python_cmd "${CONDA_ENV}"

export PYTHONDONTWRITEBYTECODE="${PYTHONDONTWRITEBYTECODE:-1}"
export PYTHONPATH="${REPO_ROOT}/src:${PYTHONPATH:-}"

echo "[timed/x86/control] robot.type=jz_robot_pin_timed"
echo "[timed/x86/control] state=udp://${STATE_BIND_IP}:${STATE_PORT} expected=${ORIN_IP}"
echo "[timed/x86/control] command=udp://${ORIN_IP}:${COMMAND_PORT} execution=${EXECUTION}"
echo "[timed/x86/control] target_action=udp://${TARGET_ACTION_BIND_IP}:${TARGET_ACTION_PORT}" \
  "expected=${TARGET_ACTION_ALLOWED_SENDER_IP}"
echo "[timed/x86/control] cameras disabled; conda_env=${CONDA_ENV} fps=${FPS}"
if [[ "${EXECUTION}" == "armed" ]]; then
  echo "JZ ROBOT PIN TIMED ARMED"
...
exec "${PYTHON_CMD[@]}" -m lerobot.scripts.lerobot_teleoperate \
  ...
  --robot.rtsp_cameras='{}' \
  --robot.enforce_camera_state_receive_skew=false \
  --robot.timing_log_every_n=0 \
  --robot.timing_sidecar=false \
  --teleop.type=jz_robot_pin_target_action \
  --teleop.id=jz_robot_pin_timed_target_action \
