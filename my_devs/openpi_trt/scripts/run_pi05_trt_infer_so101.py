...
"""Run SO101 PI0.5 inference with a TensorRT FP16 suffix engine.

Safety default: this script will not connect to the robot or send actions unless
`--confirm-control` is explicitly provided.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "SO101/SO100 PI0.5 real inference with TensorRT FP16 suffix engine. "
    ...
    parser.add_argument(
        ...
        help="Load policy and TensorRT engine, then exit before robot connection.",
    )
    parser.add_argument(
        "--confirm-control",
        action="store_true",
...
def main() -> None:
    ...
    print(f"[INFO] TensorRT suffix engine: {engine_path}")
    ...
    patch_embed_suffix_with_trt(policy, engine_path)
    preprocessor, postprocessor = load_pre_post_processors(policy_path)

    if args.check_policy_load:
        print("[INFO] CHECK_POLICY_LOAD=true, loaded policy/processors/TRT engine; exiting before robot connection.")
        return
    ...
    try:
        ...
        while True:
            if end_t is not None and time.perf_counter() >= end_t:
                print("[INFO] Reached requested run_time_s. Exiting TRT inference loop.")
                break
        ...
        if robot.is_connected:
            robot.disconnect()
        print("[INFO] TRT inference finished.")
