...
"""Run Agilex single-arm Diffusion Policy observation checks and inference.

This script mirrors the Agilex ACT single-arm helper, but targets Diffusion
Policy checkpoints trained on a single arm:
1) read full Agilex live observation
2) slice the selected arm's 7D state and matching side camera
3) optionally run Diffusion Policy inference
...
python my_devs/train/dp/agilex/run_dp_single_arm_infer.py \
    ...
    --policy-path /home/agilex/cqy/flexible_lerobot/outputs/train/20260315_214740_dp_agilex_first_test_right_full/checkpoints/100000/pretrained_model \
...
DEFAULT_POLICY_PATH = (
    ...
    "20260315_214740_dp_agilex_first_test_right_full/checkpoints/100000/pretrained_model"
)
...
DEFAULT_TASK = "Execute the trained Agilex single-arm Diffusion Policy task"
ARM_TO_STATE_NAMES = {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Agilex single-arm Diffusion Policy observation check, shadow inference, and closed-loop inference."
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "policy_inference runs Diffusion Policy and optionally publishes actions."
        ),
    )
    parser.add_argument(
        "--control-mode",
    ...
    parser.add_argument(
        ...
        help="Optional Diffusion deployment override. Must be in [1, horizon - n_obs_steps + 1].",
    )
    parser.add_argument(
        "--policy-num-inference-steps",
        type=int,
        default=parse_optional_int(os.getenv("POLICY_NUM_INFERENCE_STEPS")),
        help="Optional Diffusion deployment override. Must be > 0.",
    )
    parser.add_argument(
        "--task",
...
def apply_diffusion_runtime_overrides(
    policy_cfg: PreTrainedConfig,
    policy_n_action_steps: int | None,
    policy_num_inference_steps: int | None,
) -> None:
    if policy_cfg.type != "diffusion":
        raise ValueError(f"Expected Diffusion Policy, got {policy_cfg.type!r}")

    max_allowed_action_steps = int(policy_cfg.horizon) - int(policy_cfg.n_obs_steps) + 1
    if policy_n_action_steps is not None:
        if not 1 <= policy_n_action_steps <= max_allowed_action_steps:
            raise ValueError(
                "--policy-n-action-steps must be within "
                f"[1, {max_allowed_action_steps}], got {policy_n_action_steps}"
            )
        policy_cfg.n_action_steps = policy_n_action_steps

    if policy_num_inference_steps is not None:
        if policy_num_inference_steps <= 0:
            raise ValueError(f"--policy-num-inference-steps must be > 0, got {policy_num_inference_steps}")
        policy_cfg.num_inference_steps = policy_num_inference_steps
...
def validate_single_arm_checkpoint_schema(policy_cfg: PreTrainedConfig, arm: str) -> tuple[int, int]:
    ...
    if missing_inputs or extra_inputs or missing_outputs or extra_outputs:
        raise ValueError(
            "Checkpoint schema does not match the requested Agilex single-arm Diffusion deployment contract. "
...
def print_runtime_summary(
    ...
    print(
        "[INFO] Diffusion runtime config: "
        f"n_obs_steps={policy_cfg.n_obs_steps}, "
...
def main() -> None:
    ...
    apply_diffusion_runtime_overrides(
    ...
    try:
        bridge.connect(
            node_name="lerobot_agilex_dp_single_arm_infer",
        ...
        if bridge.is_connected:
            bridge.disconnect()
        print("[INFO] Agilex single-arm Diffusion inference finished.")
